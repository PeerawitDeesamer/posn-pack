...
MATH_TOPICS = {
    'realnum':    ('จำนวนจริง สมบัติ ค่าสัมบูรณ์ กรณฑ์ เลขยกกำลัง เศษส่วน สัดส่วน', 8, (1, 4)),
    'numtheory':  ('การหารลงตัว เศษเหลือ จำนวนเฉพาะ มอดุลาร์ เลขโดด หลักหน่วย', 16, (2, 4)),
    'polynomial': ('พหุนาม แยกตัวประกอบ สมการ/อสมการดีกรีไม่เกิน 4', 7, (2, 4)),
    'func':       ('ฟังก์ชัน โดเมน หนึ่งต่อหนึ่ง กราฟกำลังสอง ฟังก์ชันประกอบ/ผกผัน', 11, (2, 4)),
    'relprop':    ('คุณสมบัติของความสัมพันธ์ สะท้อน สมมาตร ถ่ายทอด สมมูล', 4, (2, 4)),
    'explog':     ('ฟังก์ชันเอกซ์โพเนนเชียล ลอการิทึม และกราฟ', 4, (2, 4)),
    'equation':   ('อสมการเชิงเส้น อสมการค่าสัมบูรณ์ สมการกำลังสอง ระบบสมการ', 8, (1, 4)),
    'set':        ('เซต เพาเวอร์เซต ผลคูณคาร์ทีเซียน เพิ่มเข้า-ตัดออก เซตที่มีเงื่อนไข', 8, (2, 4)),
    'logic':      ('ประพจน์ ตารางค่าความจริง ตัวบ่งปริมาณ การอ้างเหตุผล จับโกหก', 12, (2, 4)),
    'geometry':   ('เรขาคณิตวิเคราะห์ พีทาโกรัส ความคล้าย วงกลม คอร์ด เส้นสัมผัส', 4, (2, 4)),
    'coordgeo':   ('เวกเตอร์ พิกัดบนระนาบ ระยะยุคลีเดียน รูปหลายเหลี่ยม นูน/เว้า พื้นที่', 4, (2, 4)),
    'counting':   ('หลักการบวก-คูณ เรียงสับเปลี่ยน/จัดหมู่ (ของต่างกันหมด) รังนกพิราบ', 8, (2, 4)),
    'sequence':   ('ลำดับเลขคณิต ลำดับเรขาคณิต ฟิโบนักชี (ไม่รวมอนุกรม)', 6, (2, 4)),
    # ห้ามแนวอัตราเร็วเฉลี่ย ผสมสารละลาย อัตราส่วนแบ่งผลรวม งาน-คนทำงาน — ไม่มีในข้อสอบจริง
    'wordprob':   ('โจทย์ที่ต้องตีความเงื่อนไขหลายชั้น ตัวดำเนินการนิยามเอง', 3, (2, 4)),
}

COMP_TOPICS = {
    'pyexpr':    ('นิพจน์ไพธอน ลำดับดำเนินการ // % ** ชนิดข้อมูล', 11, (1, 3)),
    'pycond':    ('เงื่อนไข if/elif ซ้อน and/or/not นิพจน์ตรรกะสมมูล', 13, (1, 4)),
    'pyloop':    ('วนซ้ำ while ไล่ค่าตัวแปร เงื่อนไขหยุด', 14, (1, 4)),
    'pynest':    ('ลูปซ้อนไม่เกิน 2 ชั้น นับรอบชั้นใน วิเคราะห์ตัวคูณ', 7, (2, 4)),
    'pyarray':   ('อาเรย์ 1 มิติ/สตริง เข้าถึงด้วยดัชนี ไล่สมาชิกด้วย while', 10, (2, 4)),
    'pymix':     ('if+while ผสม กระบวนการหยุดหรือไม่หยุด', 6, (3, 4)),
    'algotrace': ('ทำตามขั้นตอนวิธี/รหัสเทียมที่โจทย์ให้ หาผลลัพธ์จากอินพุต', 14, (2, 4)),
    'algocount': ('นับจำนวนครั้งที่คำสั่ง/การพิมพ์ถูกทำงาน', 9, (2, 4)),
    'algomod':   ('เปลี่ยนขั้นตอนบางจุด หรือเทียบโปรแกรมหลายเวอร์ชัน', 9, (2, 4)),
    'simulate':  ('จำลองสถานการณ์ตามกฎที่ให้ (คิว ทรัพยากร เส้นทาง แจกของ)', 12, (2, 4)),
    'greedy':    ('เลือกที่ดีที่สุดทีละขั้น ครอบคลุมช่วง จับคู่ ค่าสุดขีด', 5, (3, 4)),
    'puzzle':    ('ปริศนาตรรกะภาษาไทย จัดลำดับ ป้ายผิด อนุมานจากเงื่อนไข', 10, (2, 4)),
}
...
LEVELS = {
    ...
    3: ('กลาง', 'ต้องมีข้อสังเกต 1 อย่าง หรือไล่หลายรอบอย่างเป็นระบบ 2-4 นาที'),
    4: ('ยาก', 'ต้องแบ่งกรณีเอง หรือคุมเงื่อนไขหลายชั้นให้ครบ 4-7 นาที'),
    5: ('ยากมาก', 'เพดานบนสุด ข้อสอบจริงมีปีละ 0-1 ข้อ — ใช้ให้น้อยที่สุด 7-12 นาที'),
}

# preset -> สัดส่วน % ของระดับ 1..5
DIFF_PRESETS = {
    'easy':     {1: 15, 2: 45, 3: 35, 4: 5,  5: 0},
    # สัดส่วนที่นับได้จริงจากข้อสอบปี 68 ทั้ง 60 ข้อ — ห้ามขยับโดยไม่นับใหม่
    'standard': {1: 5,  2: 33, 3: 47, 4: 13, 5: 2},
    'hard':     {1: 0,  2: 20, 3: 45, 4: 30, 5: 5},
    'brutal':   {1: 0,  2: 10, 3: 40, 4: 40, 5: 10},
}
DIFF_ALIAS = {
...
def build_section(name, part, fmt, count, points, diff, focus, start, rng):
    # ตอนหนึ่งมาจากคลังหัวข้อเดียว 'both' ไม่ใช่คลัง — ต้องถูกแตกเป็นสองตอนก่อนถึงตรงนี้
    # เดิมเขียนเป็น `MATH_TOPICS if part == 'math' else COMP_TOPICS` ซึ่งทำให้
    # `--part both --count 10` ได้ข้อคอมทั้ง 10 ข้อเงียบ ๆ โดยหัวตารางยังเขียนว่า
    # "คณิตศาสตร์+วิทยาการคำนวณ" ผิดแบบที่มองไม่เห็นจนกว่าจะไล่อ่านหัวข้อรายข้อ
    if part not in ('math', 'comp'):
        raise ValueError(f'build_section รับได้แค่ math/comp ไม่ใช่ {part!r}')
...
def main():
    ...
    if part == 'both' and args.count == 0:
        ...
        # ระบุจำนวนข้อเองกับ both = แบ่งครึ่งเป็นสองตอน คณิตก่อนแล้วคอม ตามโครงข้อสอบจริง
        # (จำนวนคี่ให้คณิตมากกว่า 1 ข้อ เพราะตอนคณิตมาก่อนและข้อสอบจริงคณิตมากกว่าเสมอ)
        nmath = (args.count + 1) // 2
        ncomp = args.count - nmath
        rows += build_section(f'ตอนที่ 1 คณิตศาสตร์', 'math', 'mcq',
                              nmath, 1, diff, focus, 1, rng)
        no = nmath + 1
        if fmt == 'mixed':
            nfill = args.fill_count or max(1, round(ncomp / 6))
            rows += build_section('ตอนที่ 2 วิทยาการคำนวณ แบบปรนัย', 'comp', 'mcq',
                                  ncomp - nfill, 1, diff, focus, no, rng)
            rows += build_section('ตอนที่ 3 วิทยาการคำนวณ แบบเติมคำตอบ', 'comp', 'fill',
                                  nfill, 2, diff, focus, no + ncomp - nfill, rng)
        else:
            rows += build_section('ตอนที่ 2 วิทยาการคำนวณ', 'comp', fmt, ncomp,
                                  2 if fmt == 'fill' else 1, diff, focus, no, rng)
    else:
